...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            #User Profile Creating Automatic
            profile=UserProfile()
            ...
            send_email.send()
            messages.success(request, 'Thank you for registring with us. We have sent you a verification email. Please confirm your email')
...
def login(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            messages.success(request, 'Your are no Logged In')
...
def forgotPassword(request):
    if request.method == 'POST':
        ...
        if Accounts.objects.filter(email=email).exists():
            ...
            #ResetPassword Mail configurations
            current_site = get_current_site(request)
            ...
            messages.success(request, 'We have sent you a password reset email. Please check your email')
...
def resetpassword_validate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        ...
        messages.error(request, 'Your link is invalid/expired')
        return redirect('forgotPassword')
